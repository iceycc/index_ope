jQuery.extend({
    "loadProvince": function(id, nextid) {
        ///<summary>
        /// 加载省份
        ///</summary>
		//28陕西 西安
        ...
        var ar = [5,6,7, 8, 10, 11,14, 15, 16, 20, 22, 23, 25, 26, 27, 29, 30, 31, 32, 33, 34, 35, 3358];
                ...
                if (dataResult) {
                    ...
                    for (var key in dataResult) {
						...
						// wushi fix 2015-12-01 S
                              ...
                              provinceHtml += "<option value='" + 336 + "'>" + "惠州市" + "</option>"; 
                        }
						else if(key == 28){
								...
								provinceHtml += "<option value='" + 200 + "'>" + "南京市" + "</option>";
							}
                        else if(key == 18){
                                provinceHtml += "<option value='" + 278 + "'>" + "郑州市" + "</option>";
                            }
                        else if(key == 19){
                                provinceHtml += "<option value='" + 295 + "'>" + "武汉市" + "</option>";
                            }
                        else if(key == 24){
                                provinceHtml += "<option value='" + 382 + "'>" + "成都市" + "</option>";
                            }
                        else if (key == 9){
                                provinceHtml += "<option value='" + 165 + "'>" + "大连市" + "</option>";
                            }
                        else if (key == 17){
                                provinceHtml += "<option value='" + 262 + "'>" + "青岛市" + "</option>";
                            }
        ...
        function rSelects(id, nextid, s) {
			 ///<summary>
            /// select样式初始化
            ///</summary>
            if (s == "0") {

                $("select[id=" + id + "],select[id=" + nextid + "]").each(function(index) {
                    $(this).css("display", "none");
                    creatdiv($(this));
                    rOptions($(this).attr("name"), id, nextid);
                    mouseSelects($(this).attr("name"));
					
                });
            } else if (s == "next") {
                var names = $("#" + nextid).attr("name");
                rOptions(names, id, nextid);
		...
		select_ul.appendTo(select_tag);
            ...
            function rOptions(name, id, nextid) {
				///<summary>
            	/// 获取option
            	///</summary>
                ...
                $("select[name=" + name + "] option").each(function(index) {
                    ...
                    creatli(name, txt, $(this), id, nextid);
                });
                var options_ul = 'options_' + name;
			...
			$(this).css('background-color', '');
                    ...
                    var val = $(this).val();
                    clickOptions($(this), name, val, id, nextid)
                });
            };

            function creatli(name, txt, self, id, nextid) {
				///<summary>
            	/// 创建li
            	///</summary>
                var options_ul = 'options_' + name;
        ...
        function clickOptions(self, name, val, id, nextid) {
			 ///<summary>
            /// option点击事件
            ///</summary>
